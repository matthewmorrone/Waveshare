...
I2SClass audioI2s;
#ifdef SCREEN_GPS
extern uint32_t nextGpsRefreshAtMs;
void setGpsUnavailableState(const char *updatedLabel);
void updateGps();
#endif
#ifdef SCREEN_PLANETS
extern uint32_t nextPlanetsRefreshAtMs;
...
{
  Serial.printf("Attempting HTTP time fallback: %s\n", kTimeApiFallbackUrl);

  HTTPClient http;
  if (!http.begin(kTimeApiFallbackUrl)) {
    Serial.println("HTTP time fallback setup failed");
    return false;
  }
  ...
  if (httpCode == 200) {
    ...
    if (pos != -1) {
      ...
      time_t apiTime = static_cast<time_t>(timeStr.toInt());
      if (setSystemTimeFromEpoch(apiTime)) {
        logTimeSyncState("HTTP fallback", apiTime);
        success = true;
      } else {
        Serial.printf("HTTP fallback returned invalid epoch: %ld\n", static_cast<long>(apiTime));
      }
    } else {
      Serial.println("HTTP fallback response missing unixtime");
    }
  } else {
    Serial.printf("HTTP fallback failed: HTTP %d\n", httpCode);
  }
...
void setup()
{
  ...
  if (configuredNetworkCount == 0) {
    setGpsUnavailableState("Offline - no Wi-Fi configured");
  }
  nextGpsRefreshAtMs = 0;
#endif
#ifdef SCREEN_PLANETS
  if (configuredNetworkCount == 0) {
    setPlanetsUnavailableState("Offline - no Wi-Fi configured");
  }
  ...
  buildUi();
  setDisplayBrightness(settingsState().brightness);
  autoCycleEnabled = false;  // Disable auto-cycle to prevent OOM crashes
  uint8_t savedScreen = 0;
  if (kPersistCurrentScreen) {
    savedScreen = preferences.getUChar(kPrefScreenKey, 0);
  ...
  if (!performShowScreen(savedScreen) && savedScreen != 0) {
    performShowScreen(0);
  }
  refreshUi();
  lv_obj_invalidate(lv_screen_active());
...
void loop()
{
  ...
  uint32_t now = millis();
  DebugMonitor::update(now);

  // Check if loop is hanging
  static uint32_t lastLoopLogMs = 0;
  if (now - lastLoopLogMs >= 5000) {
    lastLoopLogMs = now;
    Serial.printf("[LOOP] Running at %lums | WiFi: %s | Heap: %lu\n",
                  now, WiFi.isConnected() ? "connected" : "disconnected",
                  DebugMonitor::getFreeHeap());
  }
...
#ifdef SCREEN_WATCH
      if (activeScreen == ScreenId::Watch) {
        static uint32_t watchRefreshCount = 0;
        watchRefreshCount++;
        if (watchRefreshCount % 4 == 0) {  // Log every 4th refresh (~1 sec)
          DebugMonitor::logWatchfaceMetrics("Watch");
        }
      }
#endif
    }
  ...
  if (now - lastSkyRefreshAtMs >= kSkyRefreshMs) {
    ...
    if (activeScreen == ScreenId::Stars) {
      refreshManagedScreen(activeScreen);
    }
  }
#endif
