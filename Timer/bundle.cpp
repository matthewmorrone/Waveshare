...
void setup()
{
  ...
  if (!waveformBuildTimerScreen()) {
    Serial.println("Timer screen build failed");
    while (true) {
      delay(1000);
    }
  }

  lv_screen_load(screenRoots[static_cast<size_t>(ScreenId::Timer)]);
