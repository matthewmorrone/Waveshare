...
const ScreenModule kModule = {
    ScreenId::Stars,
    "Stars",
    waveformBuildStarsScreen,
    waveformRefreshStarsScreen,
    waveformEnterStarsScreen,
    waveformLeaveStarsScreen,
    waveformTickStarsScreen,
    waveformStarsScreenRoot,
};
} // namespace
...
lv_obj_t *buildSkyScreen()
{
  ...
  lv_label_set_text(skyUi.titleLabel, "Stars");
...
{
  screenRoots[static_cast<size_t>(ScreenId::Stars)] = buildSkyScreen();
}
...
{
  return screenRoots[static_cast<size_t>(ScreenId::Stars)];
}

bool waveformBuildStarsScreen()
{
  if (!waveformStarsScreenRoot()) {
    buildSkyScreenRoot();
  }

  return skyBuilt && waveformStarsScreenRoot() && skyUi.screen && skyUi.panel && skyUi.updatedLabel;
}
