...
constexpr size_t kStopwatchRootIndex = static_cast<size_t>(ScreenId::Stopwatch);
...
void handleStopwatchScreenTap(lv_event_t *event)
{
  LV_UNUSED(event);
  stopwatchHandleTap();
}

void handleStopwatchScreenLongPress(lv_event_t *event)
{
  LV_UNUSED(event);
...
void setup()
{
  ...
  if (!waveformBuildStopwatchScreen()) {
    Serial.println("Stopwatch screen build failed");
    while (true) {
      delay(1000);
    }
  }

  lv_obj_t *root = screenRoots[kStopwatchRootIndex];
  lv_obj_add_event_cb(root, handleStopwatchScreenTap, LV_EVENT_CLICKED, nullptr);
  lv_obj_add_event_cb(root, handleStopwatchScreenLongPress, LV_EVENT_LONG_PRESSED, nullptr);
...
{
  uint32_t now = millis();
  if (now - lastRefreshAtMs >= 50) {
    waveformRefreshStopwatchScreen();
    lastRefreshAtMs = now;
  }

  waveformTickStopwatchScreen(now);
  lv_timer_handler();
  delay(5);
}
