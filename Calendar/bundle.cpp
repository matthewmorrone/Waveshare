...
bool showScreenById(ScreenId id)
{
  ...
  if (!screenRoots[index]) {
    bool built = false;
    if (id == ScreenId::Calendar) {
      built = waveformBuildCalendarScreen();
    }
...
void setup()
{
  ...
  if (!waveformBuildCalendarScreen()) {
    Serial.println("Calendar screen build failed");
    while (true) {
      delay(1000);
    }
  }

  currentScreenIndex = static_cast<size_t>(ScreenId::Calendar);
...
void loop()
{
  ...
  uint32_t now = millis();
  if (now - lastRefreshAtMs >= 1000) {
    waveformRefreshCalendarScreen();
    lastRefreshAtMs = now;
  }

  waveformTickCalendarScreen(now);
