  ...
  for (int16_t y = moonY - glowRadius; y <= moonY + glowRadius; ++y) {
    ...
    for (int16_t x = moonX - glowRadius; x <= moonX + glowRadius; ++x) {
      ...
      if (insideCrescent) {
        ...
        color = kMoonGlow;
        alpha = static_cast<uint8_t>(42 + ((glowRadius2 - outerDist2) * 30LL) / max<int32_t>(1, glowRadius2 - moonRadius2));
      } else if (outerDist2 <= moonRadius2 && outerDist2 >= innerGlowMin2 && cutDist2 >= cutRadius2) {
        color = kMoonGlow;
        alpha = static_cast<uint8_t>(28 + ((moonRadius2 - outerDist2) * 20LL) / max<int32_t>(1, moonRadius2 - innerGlowMin2));
      } else if (outerDist2 <= moonRadius2 && cutDist2 < cutRadius2 && cutDist2 >= innerCutBand2) {
        color = kMoonGlow;
        alpha = static_cast<uint8_t>(18 + ((cutDist2 - innerCutBand2) * 24LL) / max<int32_t>(1, cutRadius2 - innerCutBand2));
      } else {
        continue;
      }
