...
std::unique_ptr<Arduino_IIC> touchController(new Arduino_FT3x68(touchBus, FT3168_DEVICE_ADDRESS, DRIVEBUS_DEFAULT_VALUE, TP_INT, nullptr));
...
constexpr size_t kCalculatorRootIndex = static_cast<size_t>(ScreenId::Calculator);
...
void setup()
{
  ...
  if (!waveformBuildCalculatorScreen()) {
    Serial.println("Calculator screen build failed");
    while (true) {
      delay(1000);
    }
  }

  lv_screen_load(screenRoots[kCalculatorRootIndex]);
...
void loop()
{
  ...
  waveformTickCalculatorScreen(now);
  lv_timer_handler();
  delay(5);
}
