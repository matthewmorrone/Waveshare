...
bool showScreenById(ScreenId id)
{
  ...
  if (!screenRoots[index]) {
    ...
    if (id == ScreenId::Watch) {
      built = waveformBuildWatchScreen();
    }
  ...
  if (static_cast<ScreenId>(currentScreenIndex) == ScreenId::Watch) {
...
void setup()
{
  ...
  if (!waveformBuildWatchScreen()) {
    Serial.println("Watch screen build failed");
    while (true) {
      delay(1000);
    }
  }

  currentScreenIndex = static_cast<size_t>(ScreenId::Watch);
