...
const ScreenModule kModule = {
    ScreenId::Geo,
    "Geo",
    waveformBuildGeoScreen,
    waveformRefreshGeoScreen,
    waveformEnterGeoScreen,
    waveformLeaveGeoScreen,
    waveformTickGeoScreen,
    waveformGeoScreenRoot,
};
} // namespace
...
bool fetchGeoData()
{
  ...
  bool success = false;
  if (http.begin(secureClient, "https://freegeoip.app/json/")) {
    ...
    if (statusCode == HTTP_CODE_OK) {
      ...
      if (!error) {
        ...
        geoState.ip = String(static_cast<const char *>(doc["ip"] | "--"));
        geoState.countryCode = String(static_cast<const char *>(doc["country_code"] | ""));
        geoState.countryName = String(static_cast<const char *>(doc["country_name"] | ""));
        geoState.regionCode = String(static_cast<const char *>(doc["region_code"] | ""));
        geoState.regionName = String(static_cast<const char *>(doc["region_name"] | ""));
        geoState.city = String(static_cast<const char *>(doc["city"] | ""));
        geoState.zipCode = String(static_cast<const char *>(doc["zip_code"] | ""));
        geoState.timeZone = String(static_cast<const char *>(doc["time_zone"] | ""));
        geoState.latitude = doc["latitude"] | 0.0f;
        geoState.longitude = doc["longitude"] | 0.0f;
        geoState.metroCode = doc["metro_code"] | 0;
        geoState.updated = weatherUpdatedLabel();
        nextGeoRefreshAtMs = millis() + kGeoRefreshIntervalMs;
        success = true;
      } else {
  ...
  if (!success) {
    if (geoState.hasData) {
      ...
      setGeoUnavailableState(networkIsOnline() ? "Retrying shortly" : "Waiting for Wi-Fi");
    }
    nextGeoRefreshAtMs = millis() + kGeoRetryIntervalMs;
  }
...
void updateGeo()
{
  ...
  if (static_cast<ScreenId>(currentScreenIndex) != ScreenId::Geo) {
    return;
  }

  if (static_cast<int32_t>(millis() - nextGeoRefreshAtMs) >= 0) {
    fetchGeoData();
    refreshGeoScreen();
  }
}
...
lv_obj_t *buildGeoScreen()
{
  ...
  lv_label_set_text(geoUi.titleLabel, "Public IP Geo");
...
void buildGeoScreenRoot()
{
  screenRoots[static_cast<size_t>(ScreenId::Geo)] = buildGeoScreen();
}
...
{
  return screenRoots[static_cast<size_t>(ScreenId::Geo)];
}

bool waveformBuildGeoScreen()
{
  if (!waveformGeoScreenRoot()) {
    buildGeoScreenRoot();
  }

  return geoBuilt && waveformGeoScreenRoot() && geoUi.screen && geoUi.ipLabel && geoUi.locationLabel &&
